#include "ImageProc.h"
#include "com_example_myndk_CameraPreview.h"
#define FPS 1
...
int initdevice(void) 
{
	...
	if (-1 == xioctl (fd, VIDIOC_TRY_FMT, &fmt)){
		LOGE("this fmt unsupport!!");
	}else{
		LOGE("this fmt support!!");
...
int initmmap(void)
{
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		LOGE("length of buf: %d", buf.length);
...
void 
Java_com_example_myndk_CameraPreview_pixeltobmp( JNIEnv* env,jobject thiz,jobject bitmap){
...
jint 
Java_com_example_myndk_CameraPreview_prepareCamera( JNIEnv* env,jobject thiz, jint videoid){
...
jint 
Java_com_example_myndk_CameraPreview_prepareCameraWithBase( JNIEnv* env,jobject thiz, jint videoid, jint videobase){
	
		int ret;

		camerabase = videobase;
	
		return Java_com_example_myndk_CameraPreview_prepareCamera(env,thiz,videoid);
	
}

void 
Java_com_example_myndk_CameraPreview_processCamera( JNIEnv* env,
										jobject thiz){
...
void 
Java_com_example_myndk_CameraPreview_stopCamera(JNIEnv* env,jobject thiz){
